...
func (q *Queries) AssignLecturerToCourse(ctx context.Context, arg AssignLecturerToCourseParams) error {
	_, err := q.exec(ctx, q.assignLecturerToCourseStmt, assignLecturerToCourse,
		arg.CourseCode,
		arg.LecturerID,
		arg.CardUid,
		arg.SchID,
...
const getActiveLecturer = `-- name: GetActiveLecturer :one
SELECT active_lecturer_id FROM courses
WHERE course_code = $1
`

func (q *Queries) GetActiveLecturer(ctx context.Context, courseCode string) (int64, error) {
	row := q.queryRow(ctx, q.getActiveLecturerStmt, getActiveLecturer, courseCode)
	var active_lecturer_id int64
	err := row.Scan(&active_lecturer_id)
	return active_lecturer_id, err
}
...
const getLecturerForAttendance = `-- name: GetLecturerForAttendance :one
SELECT card_uid, full_name, sch_id FROM course_lecturers
WHERE lecturer_id = $1
`

type GetLecturerForAttendanceRow struct {
	CardUid  string `json:"card_uid"`
	FullName string `json:"full_name"`
	SchID    string `json:"sch_id"`
}

func (q *Queries) GetLecturerForAttendance(ctx context.Context, lecturerID int64) (GetLecturerForAttendanceRow, error) {
	row := q.queryRow(ctx, q.getLecturerForAttendanceStmt, getLecturerForAttendance, lecturerID)
	var i GetLecturerForAttendanceRow
	err := row.Scan(&i.CardUid, &i.FullName, &i.SchID)
	return i, err
}

const getStudentsForAttendance = `-- name: GetStudentsForAttendance :many
SELECT card_uid, full_name, sch_id FROM course_students
WHERE course_code = $1
`

type GetStudentsForAttendanceRow struct {
	CardUid  string `json:"card_uid"`
	FullName string `json:"full_name"`
	SchID    string `json:"sch_id"`
}

func (q *Queries) GetStudentsForAttendance(ctx context.Context, courseCode string) ([]GetStudentsForAttendanceRow, error) {
	rows, err := q.query(ctx, q.getStudentsForAttendanceStmt, getStudentsForAttendance, courseCode)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []GetStudentsForAttendanceRow{}
	for rows.Next() {
		var i GetStudentsForAttendanceRow
		if err := rows.Scan(&i.CardUid, &i.FullName, &i.SchID); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
...
func (q *Queries) RegisterCourse(ctx context.Context, arg RegisterCourseParams) error {
	_, err := q.exec(ctx, q.registerCourseStmt, registerCourse,
		arg.CourseCode,
		arg.StudentID,
		arg.CardUid,
		arg.SchID,
